...
import RankConfigInfoCard from './components/rank-config-info-card.vue'
import RankConfigRewardTab from './components/rank-config-reward-tab.vue'
...
const detail = ref<RankConfig | null>(null)
const { loading, withLoading } = useDetailLoading()
const editModal = useTemplateRef<{ open: (item: RankConfig) => void }>('editModal')
...
async function fetchDetail() {
  await withLoading(async () => {
    try {
      detail.value = await rankConfigService.getById(id)
    } catch {
      router.replace({ name: RouteName.RankConfigs })
    }
  })
}
...
<template>
  <div class="flex flex-col gap-6">
    ...
    <template v-else-if="detail">
      <RankConfigInfoCard :config="detail" @edit="openEditModal" />
  ...
  <RankConfigFormModal ref="editModal" @saved="fetchDetail" />
</template>
