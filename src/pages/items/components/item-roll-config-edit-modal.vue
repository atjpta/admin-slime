...
import type { Item, RollStatConfig, ItemEquipmentMetadata } from '@/interfaces/item.interface'
...
const form = useForm({
  defaultValues: { slots: 1, stats: [] as RollStatConfig[] },
  onSubmit: async ({ value }) => {
    if (!props.item) return
    if (!value.stats.length) {
      toast.error(t('validation.required'))
      return
    }
    await itemService.updateRollConfig(props.item._id, { slots: value.slots, stats: value.stats })
    toast.success(t('item.equipment.rollConfig.updateSuccess'))
    emit('saved')
    close()
  },
})
...
watch(
  ...
  (item) => {
    ...
    const rc = meta?.rollConfig
    form.setFieldValue('slots', rc?.slots ?? 1)
    form.setFieldValue(
      'stats',
      (rc?.stats ?? []).map((s) => ({ ...s }))
    )
...
async function addStat() {
  form.pushFieldValue('stats', { stat: StatKey.ATTACK, type: StatType.FLAT, min: 0, max: 1 })
  await nextTick()
...
<template>
  <dialog ref="dialog" class="modal">
    <div class="modal-box max-w-lg">
      ...
      <h3 class="mb-4 text-lg font-semibold">{{ t('item.equipment.tabs.rollConfig') }}</h3>

      <form
        ref="listEl"
        id="item-roll-config-edit-modal-form"
        @submit.prevent="form.handleSubmit"
        class="-mx-6 max-h-[50dvh] overflow-y-auto px-6"
      >
        <form.Field
          name="slots"
          :validators="{ onChange: fieldValidator(z.number().int().min(1)) }"
        >
          <template #default="{ field }">
            <fieldset class="fieldset mb-4">
              <legend class="fieldset-legend">{{ t('item.equipment.rollConfig.slots') }}</legend>
              <input
                type="number"
                min="1"
          ...
          <form.Field name="stats" mode="array">
            <template #default="{ field: statsField }">
              <div
                ...
                :key="i"
                class="bg-base-200 grid grid-cols-[1fr_1fr_60px_60px_32px] items-center gap-2 rounded-lg p-2"
              >
      ...
      <div class="modal-action">
        <VButton
          type="button"
          form="item-roll-config-edit-modal-form"
          class="btn-ghost"
